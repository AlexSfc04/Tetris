...
class TetrisTest {
	...
	void testPieceCanMoveDown() {
		...
		assertTrue(board.canMove(piece, 0 , 1), "La pieza debería poder moverse hacia abajo.");
	}
	
	@Test
	void testPieceCannotMoveOutsideLeft() {
		Tetromino piece = new Tetromino(new Point[] {new Point(0, 0), new Point(1, 0)}, Color.BLUE);
        assertFalse(board.canMove(piece, -1 , 0), "La pieza no debería moverse fuera del borde izquierdo.");
    }
